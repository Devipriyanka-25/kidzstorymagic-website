...
function findShotType(source, action) {
  if (/\b(run|running|race|racing|jump|jumping|leap|leaping|fly|flying|float|floating|sail|sailing|steer|steering)\b/i.test(source)) {
...
function buildForegroundDetails(objects) {
  const anchors = objects.slice(0, 2);

  if (anchors.length === 0) {
    return "foreground storytelling details that lead into the page action";
  }

  return `foreground anchors like ${anchors.join(" and ")}`;
}

function buildBackgroundDetails(location, objects, sceneGuide) {
  const supportingDetail =
    objects.slice(2, 4).join(", ") ||
    cleanValue(sceneGuide?.setting || location, 100);

  return `background depth with supporting world details such as ${supportingDetail}`;
}
...
export function extractStorySceneDetails({
  ...
  const shotType = findShotType(analysisSource, action);
  const cameraAngle = findCameraAngle(analysisSource, emotion, shotType);
  const foregroundDetails = buildForegroundDetails(objects);
  const backgroundDetails = buildBackgroundDetails(location, objects, sceneGuide);
